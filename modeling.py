...
# Modified by Saeed Najafi (snajafi@ualberta.ca) for encoder-decoder generation.
# Changing the model to have a decoder transformer as well.
...
class AlbertModel(object):
    ...
    def __init__(
        ...
        if config.is_decoder:
            ...
            #  TODO: what should be the labels from input_ids without shifting?
            token_type_ids = tf.roll(token_type_ids, shift=1, axis=1)
            token_type_ids[:, 0:1] = tf.zeros(shape=[batch_size, 1], dtype=tf.int32)

            input_mask = tf.roll(input_mask, shift=1, axis=1)
            input_mask[:, 0:1] = tf.ones(shape=[batch_size, 1], dtype=tf.int32)

            input_ids = tf.roll(input_ids, shift=1, axis=1)
            input_ids[:, 0:1] = (
                tf.ones(shape=[batch_size, 1], dtype=tf.int32) * config.go_symbol_id
            )
        ...
        with tf.variable_scope(scope, default_name="bert"):
            ...
            # TODO: do I need the pooler layer + dense layer if doing encoder-decoder?
...
class EncoderDecoderAlbertModel(object):
    """Encoder Decoder model built from AlbertModel."""
    ...
    def __init__(
        ...
        Args:
          ...
          is_training: bool. true for training model, false for eval model. Controls
            whether dropout will be applied.
          input_ids: int32 Tensor of shape [batch_size, seq_length].
          output_ids: int32 Tensor of shape [batch_size, seq_length] for the output sequence.
          input_mask: (optional) int32 Tensor of shape [batch_size, seq_length].
          output_mask: (optional) int32 Tensor of shape [batch_size, seq_length].
          token_type_ids: (optional) int32 Tensor of shape [batch_size, seq_length].
          output_token_type_ids: (optional) int32 Tensor of shape [batch_size, seq_length].
          use_one_hot_embeddings: (optional) bool. Whether to use one-hot word
            embeddings or tf.embedding_lookup() for the word embeddings.
          use_einsum: (optional) bool. Whether to use einsum or reshape+matmul for
            dense layers
        ...
        with tf.variable_scope("encoder_decoder"):
            with tf.variable_scope("main_encoder"):
                ...
                encoder_hidden_output = self.encoder.sequence_output

            with tf.variable_scope("main_decoder"):
                self.decoder = AlbertModel(
                    config=config,
                    is_training=is_training,
            ...
            self.logits = dense_layer_2d(
                self.decoder.sequence_output,
                config.vocab_size,
...
def get_assignment_map_from_checkpoint(tvars, init_checkpoint, num_of_group=0):
    ...
    for name in name_to_variable:
        if name in init_vars_name:
            ...
            re.sub(r"/group_\d+/", "/group_0/", six.ensure_str(name)) in init_vars_name
            and num_of_group > 1
        ):
            tvar_name = re.sub(r"/group_\d+/", "/group_0/", six.ensure_str(name))
        elif (
            re.sub(r"/ffn_\d+/", "/ffn_1/", six.ensure_str(name)) in init_vars_name
            and num_of_group > 1
        ):
...
def dense_layer_3d_proj(
    input_tensor, hidden_size, head_size, initializer, activation, use_einsum, name=None
):
...
def dense_layer_2d(
    ...
    with tf.variable_scope(name):
        w = tf.get_variable(
            name="kernel", shape=[hidden_size, output_size], initializer=initializer
        )
        b = tf.get_variable(
...
def dot_product_attention(q, k, v, bias, dropout_rate=0.0, mask_future=False):
    ...
    if mask_future:
        future_mask = tf.zeros_like(logits)
        logits_shape = get_shape_list(logits)
        q_length, kv_length = logits_shape[-2:]
        flattern_future_mask = tf.reshape(future_mask, [-1, q_length, kv_length])

        # we assume q_length = kv_length in self-attention over decoder inputs.
        """ Example:
      if q_length = kv_length = 3:
        future_bias = [[0, -10000.0, -10000.0],
                       [0,     0,    -10000.0],
                       [0,     0,           0]]
    """
        future_bias = np.ones((q_length, kv_length)) * -10000.0
        future_bias = np.triu(future_bias)

        future_bias = tf.convert_to_tensor(future_bias, dtype=logits.dtype)
        future_bias = tf.expand_dims(future_bias, axis=0)
        flattern_future_mask = flattern_future_mask + future_bias
        future_mask = tf.reshape(flattern_future_mask, logits_shape)
        logits += future_mask
...
def attention_layer(
    ...
    Args:
      ...
      is_decoder: boolean to check if the decoder is used.
    ...
    new_embeddings = dot_product_attention(
        q, k, v, attention_mask, attention_probs_dropout_prob, mask_future=is_decoder
    )
...
def attention_ffn_block(
    ...
    if is_decoder:
        ...
        with tf.variable_scope("attention_2"):
            with tf.variable_scope("cross"):
                attention_output = attention_layer(
                    from_tensor=layer_input,
                    to_tensor=encoder_hidden_output,
